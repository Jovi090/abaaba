...
public class StockMasterEntry {
    ...
    public static void registerNewStock() {
        ...
        while (true) {
            ...
            if (isValidTicker(ticker)) {
                ...
                System.out.println("無効な銘柄コードです。もう一度入力してください。");
            }
        }
        ...
        while (true) {
            ...
            market = scanner.nextLine().trim().toUpperCase();
        ...
        try (FileWriter writer = new FileWriter(CSV_FILE_PATH, true)) {
            writer.write(String.format("\n%s,%s,%s,%d\n", ticker, productName, marketCode, sharesIssued));
    ...
    private static boolean isValidMarket(String market) {
        return market.equals("PRIME") || market.equals("STANDARD") || market.equals("GROWTH");
    }
...
public record Trade(LocalDateTime tradedDatetime, String ticker, String side, int quantity, BigDecimal tradedUnitPrice,
...
public class TradeListDisplay {
    ...
    private static final String CSV_FILE_PATH = "C:/Users/qqq/Desktop/Trade.csv";
    private static final DateTimeFormatter DATETIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
    private static final int DATETIME_WIDTH = 19; // 交易日期宽度
    private static final int TICKER_WIDTH = 7;    // 股票代码宽度
    private static final int SIDE_WIDTH = 4;      // 买卖类型宽度
    private static final int QUANTITY_WIDTH = 10; // 数量宽度
    private static final int UNIT_PRICE_WIDTH = 12; // 单价宽度
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(CSV_FILE_PATH))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (values.length == 6) {
                    ...
                    LocalDateTime inputDatetime = LocalDateTime.parse(values[5], DATETIME_FORMATTER);
                    tradeData.add(new Trade(tradedDatetime, ticker, side, quantity, tradedUnitPrice, inputDatetime));
                }
            }
        } catch (IOException e) {
            System.err.println("CSVファイルの読み込み中にエラーが発生しました: " + e.getMessage());
    ...
    private void printHeader() {
        printSeparator();
        System.out.printf("| %-"+DATETIME_WIDTH+"s | %-"+TICKER_WIDTH+"s | %-"+SIDE_WIDTH+"s | %-"+QUANTITY_WIDTH+"s | %-"+UNIT_PRICE_WIDTH+"s |%n",
                "Datetime", "Ticker", "Side", "Quantity", "Unit Price");
        printSeparator();
    }

    private void printTradeRow(Trade trade) {
        System.out.printf("| %-"+DATETIME_WIDTH+"s | %-"+TICKER_WIDTH+"s | %-"+SIDE_WIDTH+"s | %"+QUANTITY_WIDTH+"d | %"+UNIT_PRICE_WIDTH+".2f |%n",
